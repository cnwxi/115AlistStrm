...
import sqlite3
import os
from tool import readJson,getAlistMountPath,removeFile
import shutil
import tempfile
...
def preTempDB():
    ...
    temp_db_file = tempfile.NamedTemporaryFile(suffix=".db", delete=False ,dir="./data/tmp")
    ...
    with open(pathsPath, 'r', encoding='utf-16') as file:
        for line in file:
            ...
            # 插入数据到表中
            cursor.execute('INSERT INTO x_search_nodes (parent, name, is_dir, size) VALUES (?, ?, ?, 0)', (parent, name, isDir))
...
def replaceDb(tempDbPath):
    ...
    cursor.execute("INSERT INTO main.x_search_nodes (parent, name, is_dir, size) SELECT parent, name, is_dir, size FROM tempDb.x_search_nodes")
    conn.commit()
    cursor.execute("DETACH DATABASE tempDb")
    conn.commit()
    conn.close()
